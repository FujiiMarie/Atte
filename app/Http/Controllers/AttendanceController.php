...
class AttendanceController extends Controller
{
    ...
    )//ログイン時にどのボタンが押せるかの画面表示
    {
        $btn_display->start_time = false;//trueならボタン表示
        $btn_display->end_time = false;
        $btn_display->start_rest = false;
        $btn_display->end_rest = false;
            ...
            if($attendance->end_time != null){//入っている場合：全てのボタンが押せない
                ...
                if($rest->start_rest != null){//データがある場合：④休憩終了時間があるかどうか？
                    ...
                    if($rest->end_rest != null){//入っている場合（休憩終了）：「勤務終了」「休憩開始」ボタンが押せる
                        $btn_display->end_time = true;
                        $btn_display->rest_start = true;
                    }else{
                        $btn_display->rest_end = true;//入っていない場合（休憩中）：「休憩終了」ボタンが押せる    
                    }
                }else{//データがない場合：（休憩していない）：「勤務終了」「休憩開始」ボタンが押せる
                    $btn_display->end_time = true;
                    $btn_display->start_rest = true;
                }            
            }
        }else{//データがない場合:「勤務開始」ボタンが押せる
            $btn_display->start_time = true;
        }
